  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        setError(
          err.status === 401 ? "Invalid email or password. Please try again."
    ...
    <div className="w-full">
      ...
      <div className="mb-7 text-center">
        ...
        <p className="mt-2 text-sm leading-6 text-[#777]">
          Sign in to SpatialHunt — verified homes, direct to landlords.
        </p>
      </div>

      {/* Form card */}
      <div className="rounded-2xl border border-[#E0E0E0] bg-white p-8 shadow-lg">
        <form onSubmit={onSubmit} noValidate className="flex flex-col gap-5">
          ...
          {error && (
            <p role="alert" className="rounded-lg bg-red-50 px-3 py-2.5 text-sm text-red-600">
              {error}
            </p>
          )}
          ...
          <div className="flex flex-col items-center gap-2 text-sm">
            <Link href="/forgot-password" className="font-semibold text-[#1E5A4F] hover:underline">
              Forgot password?
            </Link>
