...
export default function Showcase() {
  ...
  return (
    <section className="w-full bg-white">
      <div className="mx-auto flex w-full max-w-[1400px] flex-col-reverse items-center gap-8 px-5 pt-8 pb-6 md:flex-row md:gap-10 md:px-10 lg:px-16 xl:px-20">
        ...
        <div className="flex w-full flex-col md:w-[52%]">
          ...
          <div className="mt-7 flex flex-wrap gap-4">
            <Link href="/properties">
              <button className="rounded-xl bg-[#F4B940] px-7 py-3 text-sm font-bold text-[#1E5A4F] shadow-md transition-all duration-200 hover:bg-[#e0a830] hover:shadow-lg active:scale-95">
                Find a Home
              </button>
            </Link>
            <Link href="/list-property">
              <button className="rounded-xl border-2 border-[#1E5A4F] bg-[#1E5A4F] px-7 py-3 text-sm font-bold text-[#F4B940] shadow-md transition-all duration-200 hover:bg-[#174940] hover:shadow-lg active:scale-95">
                List Your Property
              </button>
            </Link>
          </div>
            ...
            {[
              ...
              <span key={b.label} className="flex items-center gap-2 text-sm font-semibold text-[#444]">
                <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#F4B940]/15">
                  <Image src={b.icon} alt="" width={18} height={18} unoptimized />
                </span>
                {b.label}
              </span>
            ))}
          ...
          <div className="mt-5 flex items-center gap-3">
            ...
            <p className="text-sm text-[#666]">
              Join{" "}
              <span className="font-bold text-[#1E5A4F]">2,000+</span> happy
              renters &amp; landlords
            </p>
          </div>
        </div>
              ...
              {slides.map((_, i) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all duration-300 ${
                    i === current ? "w-6 bg-[#F4B940]" : "w-2 bg-white/60"
                  }`}
                />
              ))}
            </div>
          ...
          <div className="flex gap-1.5 bg-[#1E5A4F]/10 p-2">
            {slides.map((s, i) => (
              <button
                key={i}
                onClick={() => goTo(i)}
                className={`relative h-12 flex-1 overflow-hidden rounded-lg transition-all duration-200 ${
                  i === current ? "ring-2 ring-[#F4B940]" : "opacity-60 hover:opacity-90"
                }`}
              >
